...
app.delete('/api/message/:id', protectRoute, async (req, res) => {
    try {
        ...
        if (type === 'everyone') {
            ...
            // Sirf Sender hi sabke liye delete kar sakta hai
            if (msg.senderEmail === myEmail) {
                await Message.findOneAndDelete({ _id: msgId });
                // Socket se dusre bande ko batao ki delete ho gaya
                io.to(msg.receiverEmail).emit('message deleted', msgId);
                return res.json({ message: "Deleted for everyone" });
            } else {
                return res.status(403).json({ message: "Permission denied" });
            }
        } else {
            // Delete for Me (Sirf user ki list se hatao)
            await Message.updateOne({ _id: msgId }, { $push: { deletedFor: myEmail } });
...
// --- SOCKET ---
io.use((socket, next) => { const token = socket.handshake.auth?.token || socket.handshake.headers.authorization?.split(" ")[1] || socket.handshake.headers.cookie?.split('token=')[1]; if (!token) return next(new Error("Authentication error")); try { const decoded = jwt.verify(token, JWT_SECRET); socket.userEmail = decoded.email; next(); } catch (err) { next(new Error("Authentication error")); } });
io.on('connection', (socket) => {
    socket.join(socket.userEmail); socket.on('join room', (room) => socket.join(room));
    socket.on('send message', async (data) => { const { receiverId, text } = data; const senderEmail = socket.userEmail; const newMsg = new Message({ senderEmail, receiverEmail: receiverId, text, timestamp: Date.now(), status: 'sent' }); await newMsg.save(); io.to(receiverId).emit('new message', { ...data, senderEmail, status: 'received', timestamp: Date.now() }); });
});
