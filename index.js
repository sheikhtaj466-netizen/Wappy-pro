...
const userSchema = new mongoose.Schema({
    ...
    status: { type: String, default: "Offline" }, // 🔥 Online/Offline Status
    lastSeen: { type: Number, default: 0 },       // 🔥 Last Seen Time
...
app.post('/api/send-request', protectRoute, async (req, res) => {
    ...
    target.friendRequests.push({ senderEmail: sender.email, senderId: sender.wappyId, avatarUrl: sender.avatarUrl, displayName: sender.displayName }); await target.save(); io.to(targetEmail).emit('new request', { senderName: sender.displayName, senderId: sender.wappyId }); res.json({ message: "Sent" });
});
...
app.get('/api/contacts', protectRoute, async (req, res) => { 
    ...
    const detailedContacts = await Promise.all(myContacts.map(async (contact) => { 
        ...
        // Send fresh status from DB
        return { ...contact, status: friendUser.status, lastSeen: friendUser.lastSeen, avatarUrl: friendUser.avatarUrl, signature: contact.signature }; 
    })); 
    res.json(detailedContacts.filter(Boolean)); } catch (error) { res.status(500).json({ message: 'Server error' }); } 
});
...
app.delete('/api/message/:id', protectRoute, async (req, res) => {
    try { const msgId = req.params.id; const { type } = req.body; const myEmail = req.user.email;
        if (type === 'everyone') { const msg = await Message.findOne({ _id: msgId }); if (!msg) return res.status(404).json({message: "Not found"}); if (msg.senderEmail === myEmail) { msg.isDeleted = true; msg.text = "This message was deleted"; await msg.save(); io.to(msg.receiverEmail).emit('message updated', msg); return res.json({ message: "Deleted for everyone", updatedMsg: msg }); } else { return res.status(403).json({ message: "Permission denied" }); } } else { await Message.updateOne({ _id: msgId }, { $push: { deletedFor: myEmail } }); res.json({ message: "Deleted for you" }); }
...
io.on('connection', async (socket) => {
    ...
    // Broadcast to everyone (Client side will filter)
